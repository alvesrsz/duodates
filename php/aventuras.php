...
$nome_categoria = 'Aventuras e Natureza';
...
<head>
    ...
    <title>DuoDates - Aventuras e Natureza</title>
...
<body>
    ...
    <main class="container">
        <section class="page-title">
            <h2>Aventuras e Natureza em Brasília</h2>
            <p>Sugestões de lugares para encontros inesquecíveis ao ar livre na capital.</p>
        </section>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.favorite-btn').forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (isFavorited) { icon.classList.remove('far'); icon.classList.add('fas'); } else { icon.classList.remove('fas'); icon.classList.add('far'); }
                fetch('../php/salvar_favorito.php', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({place_id: placeId, is_favorited: isFavorited}) })
                .then(response => response.json()).then(data => { if (data.status === 'error') { alert(data.message); button.classList.toggle('favorited'); } })
        ...
        if (agendamentoModal) {
            ...
            agendamentoForm.addEventListener('submit', function(e) {
                ...
                fetch('../php/salvar_agendamento.php', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) })
                .then(response => response.json()).then(result => {
                    if(result.status === 'success') { alert('Date agendado com sucesso!'); closeAgendamentoModal(); agendamentoForm.reset(); } 
                    else { alert('Erro ao agendar: ' + result.message); }
