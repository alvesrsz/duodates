...
if ($view == 'dashboard') {
    ...
    // Busca direta na tabela, sem joins complicados para não dar erro 500
    $sql_pendentes = "SELECT 
                        lp.titulo as nome_local, 
                        lp.local_info,
                        lp.id AS id_local_pendente
    ...
    if ($id_local_pendente) {
        // Busca o local diretamente pelo ID sem travas de aprovação de usuário
        $sql_detalhe = "SELECT lp.* FROM locais_pendentes lp WHERE lp.id = $id_local_pendente";
        
        $result_detalhe = executarConsulta($conn, $sql_detalhe);
        
        if ($result_detalhe && $result_detalhe->num_rows > 0) {
